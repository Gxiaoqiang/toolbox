...
| **文档转 PDF** | — | — | 无选项，直接转换 |
| **Markdown 转 DOCX** | outputName | output | 输出文件名（用户可选指定） |
...
3. **可选参数引导（适用所有有选项的工具）**, 遵循"默认优先 + 按需询问":
   ...
   - 根据用户场景智能推荐:
     · "要清晰/高清" → 高 DPI + png；"要小文件/传给别人" → 低 DPI + jpeg + 低 quality
     · "简单拆分/每页拆开" → by-page；"只要某几页" → by-range
     · "尽量压缩/不在乎质量" → level 1/2；"保持原样" → level 4/5
...
Agent: "好的，有三种拆分方式:
       · 逐页拆分 — 每页一个文件 (200个)
       · 每 N 页一组 — 如每 2 页、每 5 页
       · 指定页码范围 — 如只要 1-10 页
       你选哪种？"

用户: "每 5 页一组"

Agent: "好的，每 5 页一组，200 页共产生 40 个文件。
       使用默认设置开始拆分？"
