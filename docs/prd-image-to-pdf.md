...
- **后端 Service 层**：`ImageToPdfService` 接口 + `ImageToPdfServiceImpl` 实现，职责为接收图片字节数组列表 + 配置参数，输出 PDF 字节数组
- **后端 Controller 层**：`ImageController`（`/api/image/` 命名空间），作为图片处理的统一入口，本次新增 `image-to-pdf` 端点
- **Agent 工具**：`DocAgentToolkit` 新增 `imageToPdf()` @Tool 方法
- **前端组件**：`tools/image-to-pdf/index.vue`，独立工具页面
...
遵循 `category: file` 的三栏布局（左输入 → 中操作 → 右结果）：

- **左侧**：拖拽上传区 + 图片缩略图网格预览（支持拖拽排序、删除、追加）
- **中间**：操作按钮列（方向/边距/适配/合并 参数面板 + 转换按钮）
- **右侧**：结果下载区

缩略图使用 `URL.createObjectURL()` 本地预览，不上传到服务器。排序使用 vuedraggable（与 pdf-merge 一致）。
...
1. **后端 Service 测试**：单元测试 `ImageToPdfServiceImpl`，验证各种 fitMode/margin/orientation 组合的输出
2. **Controller 测试**：验证文件校验（数量、大小、格式）、API 返回格式
3. **Agent 工具测试**：验证 `imageToPdf()` @Tool 方法的参数解析和结果格式
4. **前端测试**：上传/预览/排序/删除/参数调整的交互流程
5. **集成测试**：端到端上传图片 → 下载 PDF 的完整流程
...
- 前端组件复用 pdf-merge 的拖拽上传和排序交互模式，保持一致性
