  ...
  <div class="flex flex-col gap-4 h-full">
    <div class="flex items-center gap-3">
      ...
      <div class="relative flex-1">
        ...
        <input v-model="regexPattern" class="w-full pl-7 pr-10 py-2 border border-slate-200 rounded-lg font-mono text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400" placeholder="\d+" @input="runRegex" />
        <span class="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 text-sm">/{{ flags }}</span>
      </div>
      <div class="flex items-center gap-1">
        <button v-for="f in ['g', 'i', 'm']" :key="f" @click="toggleFlag(f)" class="px-2 py-1 text-xs rounded font-mono transition-colors" :class="flags.includes(f) ? 'bg-indigo-100 text-indigo-700' : 'bg-slate-100 text-slate-500'">{{ f }}</button>
      </div>
    </div>
    <div class="flex-1 flex gap-4">
      <div class="flex-1 flex flex-col">
        <label class="text-xs font-semibold text-slate-500 mb-2">测试文本</label>
        <textarea v-model="testText" class="flex-1 p-4 border border-slate-200 rounded-lg resize-none font-mono text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400" placeholder="输入测试文本..." @input="runRegex"></textarea>
      </div>
      <div class="flex-1 flex flex-col">
