  ...
  <div class="flex flex-col h-full">
    ...
    <header class="flex items-center gap-3 pb-3 flex-shrink-0 border-b" style="border-color: var(--border-color)">
      ...
      <template v-if="stage !== 'noFile'">
        ...
        <span class="text-xs flex-shrink-0" style="color: var(--text-muted)">共 {{ totalPages }} 页 · {{ totalRects }} 方块</span>
        ...
        <div class="flex items-center gap-1">
          <button v-for="c in colors" :key="c.value" @click="selectedColor = c.value"
    ...
    <footer class="flex items-center justify-between pt-3 flex-shrink-0 border-t" style="border-color: var(--border-color)"
      ...
      <div class="flex items-center gap-3">
        ...
        <select v-model="redactMode" class="px-2 py-1 text-xs border rounded-md"
        ...
        <span class="text-[10px]" :style="redactMode === 'deep' ? { color: '#d97706' } : { color: 'var(--text-muted)' }">
          {{ redactMode === 'deep' ? '⚠ 全页转为图片，文字将不可选中' : '提示：选中方块后可 Delete 删除，拖拽四角缩放' }}
        </span>
      </div>
      ...
      <div v-if="showConfirm" class="fixed inset-0 z-50 flex items-center justify-center bg-black/30" @click.self="showConfirm = false">
        <div class="rounded-xl shadow-xl border p-6 w-96 max-w-[90vw]" style="background: var(--bg-card); border-color: var(--border-color)">
          ...
          <ul class="text-sm space-y-1 mb-3 max-h-48 overflow-y-auto" style="color: var(--text-primary)">
            <li v-for="[page, count] in Array.from(pageRectCount.entries())" :key="page">第 {{ page }} 页：{{ count }} 个方块</li>
          </ul>
  ...
  if (previewMode.value) {
    ...
    for (const rect of rects) {
      ...
      if (rect.id === selectedRectId.value) {
        ...
        ctx.setLineDash([4, 2]); ctx.strokeRect(sx, sy, sw, sh); ctx.setLineDash([])
        const hs = 7; ctx.fillStyle = '#3b82f6'
...
function onMouseMove(e: MouseEvent) {
  ...
  if (isDrawing) {
    ...
    if (ctx) {
      ...
      ctx.fillStyle = selectedColor.value + '60'; ctx.fillRect(x, y, w, h)
      ctx.strokeStyle = selectedColor.value; ctx.lineWidth = 2; ctx.strokeRect(x, y, w, h)
    }
  ...
  if (isDrawing && activePage >= 0) {
    ...
    if (cw >= 5 && ch >= 5) {
      ...
      const newRect: RectItem = {
        ...
        color: selectedColor.value, page: activePage,
      }
      addRect(newRect)
...
async function doSubmit() {
  ...
  clearResult()
  processingLabel.value = '遮盖处理中，请稍候...'
  stage.value = 'processing'

  try {
    const allRects: any[] = []
    for (const [page, rects] of pageRects.value.entries()) {
      for (const r of rects) allRects.push({ page, x: r.x, y: r.y, w: r.w, h: r.h, color: r.color })
    }
    ...
    const resp = await fetch(`${window.location.origin}/api/pdf/redact`, { method: 'POST', body: formData })
    if (!resp.ok) { const errText = await resp.text(); throw new Error(errText || `HTTP ${resp.status}`) }

    resultBlob.value = await resp.blob()
    stage.value = 'done'
    // done 状态下重新渲染 PDF 预览，使 canvas 恢复显示
    await nextTick()
