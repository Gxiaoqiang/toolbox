  ...
  <div class="flex gap-4 h-full">
    ...
    <div class="flex-1 flex flex-col min-w-0">
      <div class="flex items-center justify-between mb-2">
        ...
        <span class="text-[10px] text-slate-400">支持 GFM 语法</span>
      </div>
      <textarea
        v-model="input"
    ...
    <div class="flex-1 flex flex-col min-w-0">
      <div class="flex items-center justify-between mb-2">
        ...
        <button v-if="activeTab === 'html'" @click="copyHtml" class="px-3 py-1 text-xs rounded-md bg-slate-100 hover:bg-slate-200 text-slate-600">
          {{ copied ? '✓ 已复制' : '复制 HTML' }}
        </button>
      </div>

      <!-- HTML 预览 -->
      <div v-if="activeTab === 'html'" class="flex-1 flex flex-col min-w-0">
        <div class="flex-1 p-4 border border-slate-200 rounded-lg overflow-auto bg-white markdown-body" v-html="htmlOutput"></div>
        <button @click="downloadHtml" class="mt-2 px-4 py-1.5 text-sm rounded-md bg-indigo-500 hover:bg-indigo-600 text-white self-start">
          下载 .html
        </button>
      </div>
      ...
      <div v-else class="flex-1 p-6 border border-slate-200 rounded-lg overflow-auto bg-white flex flex-col items-center justify-center gap-4">
        <div class="text-center">
          ...
          <p class="text-xs text-slate-400 mb-4">将 Markdown 内容转换为 .docx 文件下载</p>
        </div>
        <button @click="convertToDocx" :disabled="converting"
          class="px-6 py-2 text-sm rounded-lg transition-colors flex items-center gap-2"
          :class="converting ? 'bg-slate-200 text-slate-400 cursor-not-allowed' : 'bg-indigo-500 hover:bg-indigo-600 text-white'">
          <span v-if="converting" class="inline-block animate-spin">⟳</span>
          <span>{{ converting ? '转换中...' : '转为 DOCX 并下载' }}</span>
        </button>
        <p class="text-[10px] text-slate-400">需要后端服务支持</p>
      </div>
    </div>
  </div>
</template>
...
const tabs = [
  ...
  { key: 'docx' as const, label: 'DOCX 下载' },
]
const { copied, copy } = useClipboard()
const { success, error: toastError } = useToast()

function renderHtml() {
  try { htmlOutput.value = marked.parse(input.value) as string }
  catch { htmlOutput.value = '<p style="color:red">解析错误</p>' }
}

function copyHtml() { copy(htmlOutput.value); success('HTML 已复制') }

function downloadHtml() {
  const fullHtml = '<!DOCTYPE html><html lang="zh-CN"><head><meta charset="UTF-8"><title>Output</title><style>body{max-width:900px;margin:0 auto;padding:2rem;font-family:-apple-system,sans-serif;line-height:1.6;color:#1a1a2e}h1{border-bottom:2px solid #eee;padding-bottom:.3em}h2{border-bottom:1px solid #eee;padding-bottom:.3em}pre{background:#1e1e2e;color:#cdd6f4;padding:1rem;border-radius:8px;overflow-x:auto}code{background:#f0f0f0;padding:.2em .4em;border-radius:4px}pre code{background:none;padding:0}table{border-collapse:collapse;width:100%}th,td{border:1px solid #ddd;padding:8px 12px;text-align:left}th{background:#f5f5f5}blockquote{border-left:4px solid #0366d6;padding-left:1rem;color:#555;margin-left:0}</style></head><body>' + htmlOutput.value + '</body></html>'
  const blob = new Blob([fullHtml], { type: 'text/html;charset=utf-8' })
  const url = URL.createObjectURL(blob); const a = document.createElement('a')
  a.href = url; a.download = 'output.html'; a.click(); URL.revokeObjectURL(url)
  success('HTML 文件已下载')
}
